...
%%
function [T_run,T_stand,new_T_run,run_frac]=motion_cont_Puff(imp_bin,Fs,T_seg,T_beg)
runcont=imp_bin;
L=length(imp_bin);
rind=find(imp_bin==1);
for k=2:length(rind)
    if le((rind(k)-rind(k-1)),(2*Fs))
...
%     end
j=0;
for k=1:length(temp_T_run)
    try
        if gt((T_stand(2,k)-T_stand(1,k)),(T_beg*Fs))
