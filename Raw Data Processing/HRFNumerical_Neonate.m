function out = HRFNumerical_Neonate(s,x,Fr)
    ...
    timeShift = 3; % shift stimulus backward by 10 seconds to avoid boundary effects
    IRLength = 1000; % length of impulse response fuction, 1000 samples
    
    %% Compute HRF directly, sample by pixel
    [HRF,ImpRes] = ImpResp(s,x',Fr,timeShift,IRLength); % HRF contains timeshift data
    HRF = HRF(1:IRLength,:);
    HRF = HRF'; % pixels by samples
    ImpRes=ImpRes(1:IRLength,:);
    ImpRes=ImpRes';
...
function [H,ImpRes] = ImpResp(datain,dataout,Fr, timeShift, IRLength)
...
r = zeros(IRLength,1);
 r(1) = datain(1); % this is the first row of the Toeplitz Matrix
%c(1)=1;r(1)=1;
...
%DatainSym=S'*S; %Generate symmetric matrix of ImpResp
